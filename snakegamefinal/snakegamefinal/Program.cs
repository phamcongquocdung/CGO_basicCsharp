...
namespace CGO_Buoi06_SnakeGame
{
    class Program
    {
        ...
        int score, headX, headY, fruitX, fruitY, nTail,max=0;
        ...
        void ShowBanner()
        {
            ...
            Console.WriteLine("|-------------------------|");
            Console.WriteLine("| Qua @:  10d             |");
            Console.WriteLine("| Qua $:   5d             |");
            Console.WriteLine("| Qua *:   2d             |");
            Console.WriteLine("|-------------------------|");
        ...
        void randomQua()
        {
            ...
            int typefruitrank = rand.Next(1, 4);
            switch (typefruitrank)
            {
                case 1: typefruit = "QuaTo"; break;
                case 2: typefruit = "QuaVua"; break;
                case 3: typefruit = "QuaNho"; break;
            }
        ...
        void Update()
        {
            while (!gameOver)
            {
                ...
                if (reset) break;
                int sleepTime = SleepTime(score);
                Thread.Sleep(sleepTime);
            }
            if (gameOver) Lose();
        }
        //Dieu chinh toc do ran
        int SleepTime(int score)
        {
            if (score < 20)
            {
                return 250; // toc do binh thuong dau game
            }
            else if (score < 50)
            {
                return 100; // tang toc khi dat 20 diem
            }
            else
            {
                return 50; //tang toc khi dat 50 diem
            }
        }
        //Dieu khien phim
        void CheckInput()
        ...
        void Logic()
        {
            ...
            switch (dir)
            {
                case "LEFT":
                    headX--;
                    if (headX < 1)
                        headX = width - 2; // Wrap around to the right side
                    break;
                case "RIGHT":
                    headX++;
                    if (headX >= width - 1)
                        headX = 1; // Wrap around to the left side
                    break;
                case "UP":
                    headY--;
                    if (headY < 1)
                        headY = height - 2; // Wrap around to the bottom
                    break;
                case "DOWN":
                    headY++;
                    if (headY >= height - 1)
                        headY = 1; // Wrap around to the top
                    break;
                case "STOP":
                    {
                        while (true)
            ...
            if (headX == fruitX && headY == fruitY) //trung toa do
            {
                ...
                else if (typefruit == "QuaVua")
                {
                    score += 5;
                }
                else if (typefruit == "QuaNho")
                {
                    score += 2;
                }
        ...
        void Render()
        {
            ...
            for (int i = 0; i < height; i++)
            {
                for (int j = 0; j < width; j++)
                {
                    ...
                    else if (fruitX == j && fruitY == i) // qua
                    {
                        ...
                        Console.Write(HinhDangQua(typefruit));
                        Console.ForegroundColor = ConsoleColor.Blue;
                    }
            ...
            Console.WriteLine("hight score " + max);
        }
        char HinhDangQua(string typefruit)
        {
            switch (typefruit)
            {
                case "QuaTo": return '@';
        ...
        {
            Console.WriteLine("===SNAKE GAME===");
            Console.WriteLine("high score: " + max);
